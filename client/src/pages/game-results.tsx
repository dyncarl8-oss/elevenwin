...
function useGameFinalResults(gameId: string) {
  return useQuery<MatchResult>({
    ...
    enabled: !!gameId,
    // Disable refetching to avoid conflicts on results page
...
export default function GameResults({ gameId }: GameResultsProps) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <Button onClick={handleBackToLobby} variant="outline" data-testid="button-back-to-lobby-error">
            Back to Lobby
          </Button>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4">
        <div className="space-y-6 max-w-4xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400">Yahtzee Championship Complete</p>
          </div>
              ...
              <div className="text-center mt-4">
                {isWinner ? (
                  <div className="space-y-3">
                    <p className="text-xl text-slate-200">Incredible! You dominated the competition!</p>
                    <p className="text-lg text-green-300">🎯 You are the Yahtzee champion! 🎯</p>
                    ...
                    <p className="text-sm text-green-200">Well played! Your skills paid off! 💰</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    <p className="text-lg text-slate-300">Better luck next time!</p>
                    <p className="text-base text-slate-400">
                      Winner: <span className="font-bold text-blue-400">{winner?.username}</span> took the crown this round
                    </p>
                    <div className="flex items-center justify-center space-x-3">
                    ...
                    <p className="text-sm text-slate-400">💪 Keep practicing and you'll get them next time!</p>
                  </div>
                )}
              </div>
            </CardHeader>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="text-center">
                    <div className="text-3xl font-bold text-blue-400">#{myResult.rank}</div>
                    <div className="text-sm text-slate-400">Final Rank</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold text-purple-400">{myResult.totalScore}</div>
          ...
          <Card className="bg-gradient-to-r from-slate-800/50 to-slate-700/50 border-slate-600">
            <CardContent className="pt-6">
              <div className="text-center text-sm text-slate-400">
                ...
                <p className="mt-1">{isWinner ? "Congratulations again on your victory! Ready for another challenge?" : "Thanks for playing! Ready to get your revenge in the next game?"}</p>
              </div>
            </CardContent>
          </Card>
        </div>
